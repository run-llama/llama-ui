...
function ChatMessage(props: ChatMessageProps) {
  ...
  const minHeight = currentHeight
    ? Math.max(200, Math.floor(currentHeight * 0.8))
    : 300;
  const applyMinHeight = props.isLast && props.message.role === "assistant";
  const wrapperStyle = applyMinHeight
  ...
  return (
    ...
    >
      <div
        className={cn("group flex gap-4 p-3", props.className)}
        style={wrapperStyle}
      >
        {children}
      </div>
