...
echo "Uploading assets from ${SOURCE_DIR} to R2 bucket ${R2_BUCKET}"
...
upload_file() {
  local file="$1"
  local dest="s3://${R2_BUCKET}/$(basename "$file")"
...
JS_URL="${R2_PUBLIC_URL%/}/${JS_NAME}"
CSS_URL="${R2_PUBLIC_URL%/}/${CSS_NAME}"
